  ...
  return (
    <div>
      ...
      <header className="mb-5">
        <h1 className="text-title font-semibold text-text">Trash</h1>
      ...
      */}
      <div className="card relative md:overflow-x-auto">
        ...
        {/* Stacked below 768px, for the same reason as the file table: the
            columns a phone would push off screen are the informative ones. */}
            ...
            <li key={file.id} className="flex items-start gap-3 px-4 py-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 flex items-center gap-2">
                  <span className="tabular text-caption text-muted">{bytes(file.size_bytes)}</span>
              ...
              <div className="flex shrink-0 items-center gap-1">
                <button
                  type="button"
                  aria-label={`Restore ${file.name}`}
                  className="rounded p-2 text-muted hover:text-success"
                  onClick={() => restore.mutate(file.id)}
                >
        ...
        {!isLoading && files.length === 0 && (
          <div className="px-4 py-16 text-center">
            <p className="text-body font-semibold text-text">Trash is empty.</p>
            <p className="mt-1 text-body text-muted">
              Files you delete land here first, and keep their shards until you erase them.
            </p>
          </div>
        )}
      </div>
