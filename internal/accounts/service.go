...
// DesktopGoogleOAuthConfig builds the OAuth config for a desktop (public,
// RFC 8252) client: no client secret, and a redirect URL that is the
// caller's ephemeral loopback listener rather than a server-hosted callback
// route. Compiling a client secret into a distributed binary would defeat
// the point of it being a secret, and Google does not require one for a
// Desktop app client type — if a flow appears to need one, the client was
// registered as the wrong type.
func DesktopGoogleOAuthConfig(clientID, redirectURL string) *oauth2.Config {
	return &oauth2.Config{
		ClientID:    clientID,
		RedirectURL: redirectURL,
		Scopes:      []string{gdrive.Scope, "openid", "email"},
...
// no client secret, PKCE mandatory, and a caller-supplied config because the
